...
class Auth0 {
  ...
  Future<void> loginWithPopup(
      {PopupLoginOptions options = null, PopupConfigOptions config = null}) {
    return promiseToFuture(
        _auth0js.loginWithPopup(options ?? jsify({}), config ?? jsify({})));
  }

  void logout({LogoutOptions options = null}) =>
      _auth0js.logout(options ?? jsify({}));

  Future<String> getTokenWithPopup(
      {GetTokenWithPopupOptions options = null,
      PopupConfigOptions config = null}) {
    return promiseToFuture(
  ...
  Future<Map<String, dynamic>> getUser({GetUserOptions options = null}) =>
      promiseToFutureAsMap(_auth0js.getUser(options ?? jsify({})));
  Future<Map<String, dynamic>> getIdTokenClaims(
          {GetIdTokenClaimsOptions options = null}) =>
